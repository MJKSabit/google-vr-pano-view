    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        synchronized (this) {
            if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
                ...
                if (g==0)
                    g = Math.sqrt(event.values[0]*event.values[0]+event.values[1]*event.values[1]+event.values[2]*event.values[2]);
                verticalAngle = (float) Math.toDegrees(Math.acos(event.values[2]/g)) - 90.0f;
            ...
            if (success) {
                ...
                if (listener != null) {
                    listener.onNewAzimuth(azimuth, verticalAngle);
                }
            }
        }
    }
